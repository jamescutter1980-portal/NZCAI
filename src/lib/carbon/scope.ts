...
/**
 * Scope 1 and 2 for one asset and calendar year from stored readings.
 *
...
export function energyByMeterForYear(db: Db, meters: AssetMeter[], year: number): MeterEnergy[] {
  ...
  return meters.map((m) => {
    ...
    return { mpxn: m.mpxn, utility: m.utility, direction: m.direction, kwh: Math.round(r.kwh * 1000) / 1000, intervals: r.n, days: r.days, firstInterval: r.first ?? undefined, lastInterval: r.last ?? undefined };
  });
}

function timeVarying(db: Db, meters: AssetMeter[], year: number, region: string | undefined) {
  ...
  for (const m of meters.filter((x) => x.utility === "electricity" && x.direction === "import")) {
    for (const row of stmt.all(region, m.mpxn, from, to) as { kwh: number; g: number | null }[]) {
      if (row.g === null || row.g === undefined) unmatched += row.kwh;
      else {
        matched += row.kwh;
        g += row.kwh * row.g;
      }
  ...
  return {
    ...
    detail:
      total === 0
        ? "No electricity import readings in this year."
        : matched === 0
...
export function assetCarbon(db: Db, ctx: OperationContext, meters: AssetMeter[], year: number, gridRegion?: string): AssetCarbon {
  const energy = energyByMeterForYear(db, meters, year);
  ...
  for (const e of elecImport) {
    ...
    if (meter?.supplierFactorKgCo2ePerKwh !== undefined) {
      ...
      scope2Market.push(line(`Residual mix for ${e.mpxn}`, e.kwh, residualMixFactor(ctx, year, "GB"), [e.mpxn]));
    }
  }

  for (const f of [gasFactor, elecFactor, tdFactor]) if (f.basis === "unavailable" && f.detail) warnings.push(f.detail);
  const now = ctx.now();
  const daysInYear = year === now.getUTCFullYear() ? Math.ceil((now.getTime() - Date.UTC(year, 0, 1)) / 86_400_000) : Date.UTC(year + 1, 0, 1) === Date.UTC(year, 0, 1) + 366 * 86_400_000 ? 366 : 365;
  const missingData = energy.filter((e) => e.days > 0 && e.days < daysInYear - 5);
  if (missingData.length) warnings.push(`Partial year for ${missingData.map((e) => `${e.mpxn} (${e.days} of ${daysInYear} days)`).join(", ")}.`);
  if (energy.length > 0 && energy.every((e) => e.intervals === 0)) warnings.push(`No readings stored for ${year} on the linked meters.`);
  ...
  return {
    ...
    scope2Market,
    scope3TandD,
    timeVarying: timeVarying(db, meters, year, gridRegion),
