...
describe("google-solar", () => {
  it("puts the key in the query string and maps segments and configs", async () => {
    const fetch = vi.fn(async () => new Response(JSON.stringify(insights), { status: 200 }));
  ...
  it("treats NOT_FOUND as an empty result", async () => {
    const fetch = vi.fn(async () => new Response(JSON.stringify({ error: { code: 404, message: "Requested entity was not found.", status: "NOT_FOUND" } }), { status: 404 }));
  ...
  it("throws on a 403 (bad or missing key on the server side)", async () => {
    const fetch = vi.fn(async () => new Response(JSON.stringify({ error: { code: 403, message: "The request is missing a valid API key.", status: "PERMISSION_DENIED" } }), { status: 403 }));
  ...
  it("health check reports the upstream status without a key", async () => {
    const fetch = vi.fn(async () => new Response("{\"error\":{\"code\":403}}", { status: 403 }));
