...
export function buildValueChainExport(db: Db, ctx: OperationContext, year: number): ExportTable {
  ...
  const rows: CsvValue[][] = report.counterparties.map((v) => [
    ...
    v.report?.allocationMethod ?? (v.dataSource === "ledger" ? "activity_ledger" : ""),
    v.report?.allocationDetail ?? (v.dataSource === "ledger" ? `${v.ledger.counts.resolved} of ${v.ledger.counts.lines} ledger lines converted with the share of each line applied.` : ""),
