...
import { ASK_LABELS, ROLES, SCOPE3_CATEGORIES, STATE_LABELS, TIER_LABELS, type EngagementState, type Tier } from "@/lib/value-chain/types";
...
interface Report {
  ...
  counterparties: View[];
  coverage: { total: number; active: number; upstream: number; downstream: number; both: number; asked: number; withData: number; verified: number; declined: number; unreachable: number; dueNow: number; overdue: number; valueTotalGbp: number; valueWithDataGbp: number; valueCoveredPct: number | null; unvalued: number };
  totals: { attributableTco2e: number | null; resolved: number; unresolved: number; primarySharePct: number | null; byDirection: { upstream: number | null; downstream: number | null }; byCategory: { category: string; label: string; counterparties: number; withData: number; tco2e: number | null }[]; byTier: Record<Tier, number> };
...
export default function ValueChainPage() {
  ...
  const [form, setForm] = useState({ name: "", roles: ["supplier"] as string[], ghgCategories: [] as string[], annualValueGbp: "", contactName: "", contactEmail: "", escalationName: "", escalationEmail: "", ask: "annual_ghg_report", sector: "" });
  ...
  async function add(e: FormEvent) {
    e.preventDefault();
    const body = { ...form, ghgCategories: form.ghgCategories.length ? form.ghgCategories : undefined, annualValueGbp: form.annualValueGbp || undefined };
    if (await send("Add", "/api/value-chain/counterparties", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) })) {
      setForm({ ...form, name: "", annualValueGbp: "", contactName: "", contactEmail: "", escalationName: "", escalationEmail: "", sector: "" });
    }
  }
  ...
  const rows = (report?.counterparties ?? []).filter((v) => {
    ...
    if (filter === "no_data") return v.dataSource === "none" && v.counterparty.status === "active";
    return true;
  });
    ...
    <>
      ...
      {report && (
        <section style={card}>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            ...
            <Stat label="Data returned" value={`${report.coverage.withData} of ${report.coverage.active}`} sub={`${report.coverage.verified} verified`} />
            ...
            <Stat label="Attributable tCO2e" value={fmtN(report.totals.attributableTco2e)} sub={`${report.totals.resolved} resolved, ${report.totals.unresolved} unresolved`} />
      ...
      {report && report.counterparties.length > 0 && (
        <section style={card}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ borderCollapse: "collapse", width: "100%" }}>
              <thead><tr>{["Counterparty", "Direction", "Categories", "Value", "Engagement", "Data", "tCO2e", "Tier", "Next", ""].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
              <tbody>
                {rows.map((v) => (
                  <tr key={v.counterparty.id} style={v.counterparty.status === "inactive" ? { color: "#888" } : undefined}>
                    ...
                    <td style={td}>{{ report: "Annual report", ledger: "Ledger", none: "None" }[v.dataSource]}</td>
                    <td style={td}>{v.dataSource === "none" ? "—" : fmtN(v.attributableTco2e)}</td>
